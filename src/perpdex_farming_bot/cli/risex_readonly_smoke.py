...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare and optionally run RiseX read-only smoke checks. This never sends orders.",
    )
    ...
    parser.add_argument("--network", action="store_true", help="Actually call RiseX read-only REST endpoints.")
    ...
    parser.add_argument(
        "--public-check",
        choices=("markets", "orderbook"),
        default="markets",
        help="Public read-only endpoint to call when --public is used.",
    )
    parser.add_argument(
        "--private-check",
        choices=("balance", "positions"),
        default="positions",
        help="Private read-only endpoint to call when --private-readonly is used.",
    )
    parser.add_argument("--market-id", type=int, default=1, help="RiseX numeric market ID, e.g. 1 for BTC-PERP.")
    parser.add_argument("--limit", type=int, default=5, help="Small orderbook level/page limit.")
    parser.add_argument("--token", default="", help="Optional token address for balance checks. Value is never printed.")
    ...
    print("risex_readonly_smoke=prepared")
    ...
    try:
        ...
        print("risex_readonly_smoke_ready=False")
    ...
    if not args.network:
        print("network_skipped=pass_--network_to_call_risex_readonly_rest")
        print("risex_readonly_smoke_ready=True")
        return
    ...
    print(f"risex_readonly_smoke_ready={all_ok}")
    if not all_ok:
        raise SystemExit(1)
...
def _run_private_readonly_check(api_endpoint: str, args: argparse.Namespace) -> bool:
    params = read_risex_private_readonly_params(args.credential_prefix, args.environment)
    if args.private_check == "balance":
        ...
        path = "/v1/positions"
        query = {"account": params["account"], "page_size": args.limit}
    else:
