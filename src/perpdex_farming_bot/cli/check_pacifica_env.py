...
from perpdex_farming_bot.credentials import (
    ...
    pacifica_private_readonly_missing,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check Pacifica local env values without printing secrets.")
    ...
    print("pacifica_env_check=local_only_no_network")
    ...
    print("auth_scheme=ed25519_signature_for_post_requests")
    ...
    private_missing = pacifica_private_readonly_missing(args.credential_prefix, environment)
    signing_missing = pacifica_signing_missing(args.credential_prefix, environment)
    print("pacifica_public_readonly_env_ready=True")
    print(f"pacifica_private_readonly_env_ready={available_env is not None}")
    print(f"pacifica_signing_env_ready={not signing_missing}")
    if private_missing:
        print("private_readonly_missing_required=" + ",".join(private_missing))
    if signing_missing:
        print("signing_missing_required=" + ",".join(signing_missing))

    print(f"pacifica_env_ready={not endpoint_errors and available_env is not None}")
    if endpoint_errors:
